...
char* mysql_pesq_char(char *comando) {
    int res, i;
    char *resultado = malloc(sizeof(char) * 500);
...
int mysql_pesq_int(char *comando) {
    int res, i, valor;
    MYSQL_RES *pesq;
    MYSQL_ROW linhas;
    res=mysql_query(&cnx, comando);
...
int ini_alle() {
    ...
    fonte_movs = al_load_font("neoletters.ttf", 16, 0);
    if (!fonte){
        al_destroy_display(janela);
        error_msg("Falha ao carregar fonte");
        return -1;
    ...
    tela_x = GetSystemMetrics(SM_CXSCREEN)/2;
    tela_y = (GetSystemMetrics(SM_CYSCREEN)/2)-(GetSystemMetrics(SM_CYSCREEN)*0.1);
    return 1;
}
...
int ini_parametros() {
    ...
    al_set_window_position(janela, (tela_x-(LARG_TELA/2)), (tela_y-(ALT_TELA/2)));
	...
	al_draw_text(fonte_movs, al_map_rgb(207, 207, 207), (LARG_TELA - 70), 10, ALLEGRO_ALIGN_CENTRE, "Movimentos: 0");
...
void ini_cax() {
    ...
    for(j=0;j<4;j++) {
        ...
        case 1:
            ...
            strcpy(bpj, mysql_pesq_char(comando));
            i=0;
            do {
                if(bpj[i]!=',' && bpj[i]!='\0') {
                    aux[cont2]=bpj[i];
        ...
        case 2:
            ...
            strcpy(pi, mysql_pesq_char(comando));
            i=0;
            do {
                if(pi[i]!=',' && pi[i]!='\0') {
                    aux[cont2]=pi[i];
        ...
        case 3:
            ...
            strcpy(pj, mysql_pesq_char(comando));
            i=0;
            do {
                if(pj[i]!=',' && pj[i]!='\0') {
                    aux[cont2]=pj[i];
...
int menu() {
    ...
    al_set_window_position(janela, (tela_x-320), (tela_y-240));
...
int exist_user() {
    int quant, res, tec, y=100, i;
    MYSQL_RES *pesq;
    MYSQL_ROW linhas;
    quant=mysql_pesq_int("select COUNT(*) from tb_user");
    janela=al_create_display(640,480);
    if (!janela){
        error_msg("Falha ao criar janela");
        return 0;
	...
	while (!ent && !sair){
        while(!al_is_event_queue_empty(fila_eventos)){
            ...
            if(flag) {
                ...
                larg_mini=mysql_pesq_int(comando);
                al_draw_bitmap(fmenu,0,0,0);
...
int main(void) {
    ...
    mysql_init(&cnx);
    if(mysql_real_connect(&cnx,HOST,USER,PASS,DB,0,NULL,0)){
        do {
            usr=user();
            if(usr==-1) {
                return fechar();
